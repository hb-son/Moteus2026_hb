...
if (-not $python) {
    Write-Host "Python이 없습니다. winget으로 Python 설치를 시도합니다."

    $winget = Get-Command winget -ErrorAction SilentlyContinue
    if (-not $winget) {
        Write-Host "winget이 없습니다. Microsoft Store에서 Python 3를 먼저 설치하세요."
        exit 1
    }
...
python -m venv "$HOME\moteus-venv"

# 4. 가상환경 활성화
& "$HOME\moteus-venv\Scripts\Activate.ps1"
